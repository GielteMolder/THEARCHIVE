...
export default function App() {
  ...
  const handleQuickEdit = async (e, id) => {
    e.stopPropagation();
    try {
  ...
  const headerTitle = view === 'admin' ? 'TERMINAL_INPUT' : view === 'account' ? 'THE_IDENTITY' : 'THE ARCHIVE';
    ...
    <div className={`min-h-screen font-mono selection:bg-pink-200 selection:text-black flex flex-col transition-colors duration-500 ${darkMode ? 'bg-black text-white' : 'bg-[#f0f0f0] text-[#1a1a1a]'}`}>
      ...
      <header className={`p-4 md:p-10 border-b-8 border-black sticky top-0 z-40 shadow-[0_4px_0_0_rgba(0,0,0,1)] transition-colors ${darkMode ? 'bg-[#111]' : 'bg-white'}`}>
        <div className="flex flex-row justify-between items-center gap-2">
          <div className="group cursor-pointer select-none flex-grow" onClick={() => view === 'grid' ? setShowFilters(!showFilters) : navigateTo('grid')}>
            <h1 className="text-xl md:text-6xl font-black uppercase tracking-tighter italic leading-none transition-transform active:scale-95">
              {headerTitle}
            </h1>
            <p className={`text-[7px] md:text-xs font-bold px-2 py-0.5 mt-1 inline-block uppercase tracking-widest ${darkMode ? 'bg-white text-black' : 'bg-black text-white'}`}>
          ...
          <div className="flex flex-row gap-2 md:gap-4 items-stretch flex-shrink-0">
            {view !== 'grid' && (
              <button 
                ...
                className="btn-brutal bg-black text-white border-2 md:border-4 border-black px-2 md:px-6 py-1 md:py-3 text-[8px] md:text-xs font-black uppercase shadow-[3px_3px_0_0_rgba(0,0,0,1)] hover:bg-yellow-300 hover:text-black"
              >
                Back_
              </button>
            )}

            <button onClick={() => setDarkMode(!darkMode)} className={`btn-brutal px-3 md:px-6 py-1 md:py-3 border-2 md:border-4 border-black shadow-[3px_3px_0_0_rgba(0,0,0,1)] flex items-center justify-center ${darkMode ? 'bg-white text-black' : 'bg-black text-white'}`}>
              {darkMode ? '☼' : '☾'}
            </button>
            
            {isAdmin && view === 'grid' && (
              <button onClick={() => navigateTo('admin')} className="btn-brutal border-2 md:border-4 border-black px-2 md:px-6 py-1 md:py-3 text-[8px] md:text-xs font-black uppercase shadow-[3px_3px_0_0_rgba(0,0,0,1)] bg-yellow-300 text-black">
                + NEW
              </button>
            )}

            {!user ? (
              <button onClick={login} className={`btn-brutal border-2 md:border-4 border-black px-2 md:px-6 py-1 md:py-3 text-[8px] md:text-xs font-black uppercase shadow-[3px_3px_0_0_rgba(0,0,0,1)] ${darkMode ? 'bg-white text-black' : 'bg-black text-white'}`}>LOGIN</button>
            ) : (
              <div className="relative" ref={accountMenuRef}>
                <button onClick={() => setShowAccountMenu(!showAccountMenu)} className={`btn-brutal flex items-center gap-2 md:gap-4 px-2 md:px-4 py-1 md:py-3 border-2 md:border-4 border-black shadow-[3px_3px_0_0_rgba(0,0,0,1)] h-full ${darkMode ? 'bg-white text-black' : 'bg-black text-white'}`}>
                ...
                {showAccountMenu && (
                  <div className={`absolute right-0 mt-2 w-48 md:w-64 border-4 border-black p-4 md:p-6 z-50 shadow-[6px_6px_0_0_rgba(0,0,0,1)] flex flex-col gap-4 animate-in fade-in slide-in-from-top-2 duration-200 ${darkMode ? 'bg-[#111] text-white' : 'bg-white text-black'}`}>
                    <div className="flex flex-col border-b-2 border-black/10 pb-3 uppercase">
                      <span className="text-[8px] font-black opacity-40 mb-1 tracking-widest italic">Identity_</span>
                      <span className="text-[9px] md:text-[12px] font-bold truncate lowercase">{user.email}</span>
                    </div>
                    <button onClick={() => navigateTo('account')} className="btn-brutal w-full bg-blue-500 text-white text-[10px] md:text-xs font-black uppercase py-2 md:py-3 border-2 border-black shadow-[3px_3px_0_0_rgba(0,0,0,1)]">My_Profile_</button>
                    <button onClick={logout} className="btn-brutal w-full bg-red-500 text-white text-[10px] md:text-xs font-black uppercase py-2 md:py-3 border-2 border-black shadow-[3px_3px_0_0_rgba(0,0,0,1)]">Sign_Out_</button>
                  </div>
                )}
              </div>
            )}
        ...
        {view === 'grid' && (
          <div className={`filters-container ${showFilters ? 'filters-active mt-8' : ''}`}>
            <div className="flex flex-col gap-6 p-1 border-t-4 border-black pt-6">
              <div className={`flex border-4 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] max-w-xl ${darkMode ? 'bg-black border-white' : 'bg-white'}`}>
                <input type="text" placeholder="SEARCH / #TAGS..." className="bg-transparent p-3 text-sm font-bold outline-none flex-grow uppercase placeholder:opacity-30" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
              <div className="flex flex-wrap gap-2 md:gap-4 text-[10px] md:text-[12px] font-black uppercase">
                {['all', 'blog', 'art'].map(f => (
                  <button key={f} onClick={() => setActiveFilter(f)} className={`btn-brutal border-2 md:border-4 border-black px-3 py-1 md:px-4 md:py-2 shadow-[2px_2px_0_0_rgba(0,0,0,1)] ${activeFilter === f ? (darkMode ? 'bg-white text-black' : 'bg-black text-white') : (darkMode ? 'bg-[#222] text-white' : 'bg-white text-black')}`}>
          ...
          <div className={`p-4 md:p-12 animate-in fade-in duration-500 ${darkMode ? 'bg-black text-white' : 'bg-[#f0f0f0] text-black'}`}>
            <div className="max-w-[1400px] mx-auto w-full flex flex-col gap-12">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className={`border-4 md:border-8 border-black p-6 md:p-10 shadow-[10px_10px_0_0_rgba(0,0,0,1)] ${darkMode ? 'bg-[#111]' : 'bg-white'}`}>
                  <form onSubmit={handleSavePost} className="space-y-8">
                    <div className="flex border-4 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
                    ...
                    <div className="space-y-6 text-black">
                      ...
                      {formData.type === 'art' && ( <input type="text" placeholder="IMAGE_URL_ (HTTPS://...)" className={`w-full bg-transparent border-b-4 border-black p-2 text-lg font-black outline-none ${darkMode ? 'text-white border-white' : 'text-black'}`} value={formData.src} onChange={e => setFormData({...formData, src: e.target.value})} required /> )}
                    ...
                    <button type="submit" className="btn-brutal w-full bg-black text-white py-6 font-black text-3xl uppercase italic tracking-tighter shadow-[8px_8px_0_0_rgba(0,0,0,1)]">PUSH_TO_ARCHIVE_</button>
                  </form>
                </div>

                <div className="hidden lg:block space-y-6">
                  <span className="text-xs font-black uppercase opacity-40 italic tracking-widest">Live_Monitor_</span>
                  <div className={`border-4 border-black shadow-[8px_8px_0_0_rgba(0,0,0,1)] p-8 ${formData.isTitle ? 'bg-white' : formData.type === 'blog' ? 'bg-yellow-300' : 'bg-black'} text-black`}>
            ...
            <div className="max-w-[1200px] mx-auto w-full space-y-12">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                 <div className={`p-8 border-4 border-black shadow-[8px_8px_0_0_rgba(0,0,0,1)] flex flex-col items-center text-center ${darkMode ? 'bg-[#111]' : 'bg-white'}`}>
                    {user?.photoURL && <img src={user.photoURL} className="w-24 h-24 md:w-32 md:h-32 grayscale border-4 border-black mb-6" alt="User" />}
                    ...
                    <div className="w-full space-y-2 border-t border-black/10 pt-4">
                       <div className="flex justify-between text-[10px] font-bold uppercase"><span>Likes:</span><span className="text-pink-500">{likedPosts.length}</span></div>
                       <div className="flex justify-between text-[10px] font-bold uppercase"><span>Status:</span><span className="text-green-500">Connected</span></div>
                    </div>
                 </div>
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                        {likedPosts.length === 0 ? (
                          ...
                          <div key={post.id} onClick={() => setSelectedPost(post)} className="aspect-square border-2 border-black bg-white cursor-pointer group hover:shadow-[4px_4px_0_0_rgba(0,0,0,1)] transition-all overflow-hidden relative">
                            {post.type === 'art' ? <img src={post.src} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" alt="art" /> :
                              <div className="p-4 flex flex-col justify-center h-full bg-yellow-300"><p className="text-[8px] font-black line-clamp-3 text-black">{post.content}</p></div>
                            }
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all"><span className="text-white font-black text-[10px] uppercase tracking-widest">Open_</span></div>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <main className="p-4 md:p-12 mb-10 block flex-grow animate-in fade-in duration-700">
            <div className="columns-2 md:columns-3 lg:columns-4 xl:columns-5 gap-4 md:gap-8 max-w-[1700px] mx-auto space-y-4 md:space-y-8">
              {filteredPosts.map((post) => (
                <div key={post.id} onClick={() => setSelectedPost(post)} className={`break-inside-avoid border-2 md:border-4 border-black group cursor-pointer transition-all mb-4 md:mb-8 shadow-[4px_4px_0_0_rgba(0,0,0,1)] md:shadow-[6px_6px_0_0_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 relative ${post.type === 'art' ? 'bg-black p-0.5' : 'p-4 md:p-8'} ${post.isTitle ? 'bg-white border-l-[15px] border-black text-black' : post.type === 'blog' ? 'bg-yellow-300 text-black' : ''}`}>
                  {isAdmin && (
                    <div className="absolute -top-3 -right-3 flex gap-1 z-10">
                      <button onClick={(e) => startEditing(e, post)} className="bg-blue-500 text-white w-7 h-7 border-2 border-black flex items-center justify-center hover:bg-black transition-colors shadow-[2px_2px_0_0_rgba(0,0,0,1)]">✎</button>
                    ...
                    <div className="space-y-2 text-black">
                      {editingPostId === post.id ? (
                        <div onClick={e => e.stopPropagation()} className="space-y-2">
                          <textarea value={editContent} onChange={e => setEditContent(e.target.value)} className="w-full p-2 text-[10px] font-bold border-2 border-black bg-white text-black min-h-[100px]" />
                          <button onClick={(e) => handleQuickEdit(e, post.id)} className="bg-black text-white px-2 py-1 text-[8px] font-black uppercase btn-brutal border border-white">Save_</button>
                        </div>
                      ) : (
      ...
      <footer className={`w-full px-6 py-8 border-t border-black/10 text-[9px] md:text-[10px] font-mono opacity-40 hover:opacity-100 transition-opacity flex flex-col md:flex-row justify-between items-center gap-4 uppercase ${darkMode ? 'text-white' : 'text-black'}`}>
        ...
        <div className={`flex items-center gap-6 tabular-nums ${darkMode ? 'text-white' : 'text-black'}`}>
          ...
          <div className="bg-black text-white px-2 py-1 border border-white/20 flex items-center gap-2">
            ...
            <span className="font-black text-white">NODE_01</span>
          </div>
        </div>
      </footer>
      ...
      {selectedPost && (
        <div className="fixed inset-0 bg-black/98 z-50 flex items-center justify-center p-2 md:p-12 backdrop-blur-2xl animate-in fade-in duration-300" onClick={() => setSelectedPost(null)}>
          <div className={`w-full max-w-4xl max-h-[90vh] overflow-y-auto border-4 md:border-[10px] border-black p-6 md:p-12 relative shadow-[15px_15px_0_0_rgba(0,0,0,1)] ${darkMode ? 'bg-[#111] text-white' : 'bg-white text-black'}`} onClick={e => e.stopPropagation()}>
            <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-10 border-b-4 md:border-b-8 border-black pb-8 text-black">
              <div className="flex-grow">
                ...
                <div className="flex flex-wrap gap-2">{selectedPost.tags?.split(',').map(tag => (<span key={tag} className="text-[10px] font-black uppercase bg-black text-white px-2 py-0.5">{tag.trim()}</span>))}</div>
              </div>
              <button onClick={() => setSelectedPost(null)} className="btn-brutal bg-black text-white px-6 py-2 font-black border-2 border-black hover:bg-yellow-300 hover:text-black uppercase shadow-[3px_3px_0_0_rgba(0,0,0,1)]">Close_</button>
            </div>
            ...
            <div className="space-y-12">
              ...
              {selectedPost.audioSrc && (
                <div className="p-4 border-4 border-black bg-white shadow-[6px_6px_0_0_rgba(0,0,0,1)] space-y-2">
                  <p className="text-[10px] font-black uppercase italic tracking-widest text-black">// AUDIO_STREAM_ATTACHED</p>
              ...
              <div className="mt-20 border-t-8 border-black pt-12">
                ...
                <div className="space-y-6 mb-12">
                  {comments.length === 0 ? <p className="text-xs font-bold opacity-30 italic text-black uppercase">No signals yet...</p> : 
                    comments.map(c => (
                      <div key={c.id} className={`p-4 border-4 border-black flex gap-4 ${c.isAdmin ? 'bg-yellow-300 text-black' : (darkMode ? 'bg-[#222]' : 'bg-white text-black')} shadow-[4px_4px_0_0_rgba(0,0,0,1)]`}>
                ...
                {user ? (
                  <form onSubmit={handleAddComment} className="flex flex-col gap-4">
                    ...
                    <button type="submit" className="btn-brutal bg-black text-white py-6 font-black uppercase text-xl md:text-3xl italic tracking-tighter shadow-[8px_8px_0_0_rgba(0,0,0,1)]">Push_Response_</button>
                  </form>
                ) : (
